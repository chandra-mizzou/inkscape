...
launch_tmux() {
  ...
  echo "  Switch : Ctrl-b then 0..$(( ${#TITLES[@]} - 1 ))   (or n / p)"
  echo ""
  echo "IMPORTANT: Gazebo + RViz opening is normal — the stack is NOT idle."
  echo "  • Check T4 (bridge) and T6 (VINS) windows for errors / 'waiting for topics'."
  echo "  • In QGC: Arm → Takeoff → fly a slow figure-8 (VINS needs motion)."
  echo "  • Status:  ${SCRIPT_DIR}/check_status.sh"
  echo "  • Takeoff: ${SCRIPT_DIR}/10_demo_takeoff.sh"
...
echo ""
echo "If it 'does nothing' after Gazebo/RViz open:"
echo "  ${SCRIPT_DIR}/check_status.sh"
echo "  tmux attach -t vins-fusion-vio     # then Ctrl-b 3 / 5 to inspect T4 / T6"
echo "  ${SCRIPT_DIR}/10_demo_takeoff.sh   # or Arm+Takeoff in QGC"
echo ""
echo "Useful checks:"
